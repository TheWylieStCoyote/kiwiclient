...
class KiwiIQWavReader(collections.Iterator):
    ...
    def _proc_chunk_fmt(self, chunk):
        wFormatTag, nchannels, self._samplerate, dwAvgBytesPerSec, wBlockAlign = struct.unpack(
            '<HHLLH', chunk.read(chunk.getsize() - 2))
        assert wFormatTag == 1 and nchannels == 2 and wBlockAlign == 4, 'this is not a KiwiSDR IQ wav file'

    def _proc_chunk_kiwi(self, chunk):
        self.last_gps_solution, dummy, gpssec, gpsnsec = struct.unpack(
            '<BBII', chunk.read(10))
        self.gpssec = gpssec + 1e-9 * gpsnsec

    def _proc_chunk_data(self, chunk):
        t = None
        z = np.frombuffer(chunk.read(chunk.getsize()), dtype=np.int16).astype(
            np.float32).view(np.complex64) / 65535
        n = len(z)
        if self._last_gpssec >= 0:
            if self._frame_counter < 3:
                self._samplerate = n / (self.gpssec - self._last_gpssec)
            else:
                self._samplerate = 0.9 * self._samplerate + \
                    0.1 * n / (self.gpssec - self._last_gpssec)
